...
\subsection{Sensor}
When a message is translated by the coordinator the journey continues to the AP2DX Sensor module. This module creates a visualization of sonar and laser sensor-data in a user interface window, so it is possible to see what the robot sees. Then the message is cloned: it is sent to the Mapper module and the Reflex module.

\subsection{Mapper}
In the mapper module the odometry and laser data come together to create a map with DP-Slam. DP-Slam is a SLAM (simultaneous localization and mapping) algorithm, implemented in C. It creates an image of the environment that is only useful for the human eye. The robot can not use it to navigate the environment. The Mapper relays all sensor data to the planner.
\todo{reference to DP-slam, http://www.cs.duke.edu/~parr/dpslam2.pdf?}
